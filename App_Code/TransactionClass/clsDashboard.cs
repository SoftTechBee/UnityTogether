...
public class clsDashboard
{
    ...
    public string PlanType(string username,string PlanType)
    {
        string total = "0";
        try
        {
            if (username == "Admin")
            {
                string sql = "select sum(CONVERT(decimal(18,2),ISNULL(credit,0))) as amount from account where plantype ='" + PlanType + "'";
                DataTable dt = objcon.ReturnDataTableSql(sql);
                if (dt.Rows.Count > 0)
                {
                    total = dt.Rows[0][0].ToString();
            ...
            {
                string sql = "select sum(CONVERT(decimal(18,2),ISNULL(credit,0))) as amount from account where username='" + username + "' and  plantype like'" + PlanType + "'";
                DataTable dt = objcon.ReturnDataTableSql(sql);
                if (dt.Rows.Count > 0)
                {
                    total = dt.Rows[0][0].ToString();
  ...
  public string DepositDebit(string username,string type)
    {
        ...
        try
        {
            string sql = "select sum(debit) as amount from [tblwallet] where username='" + username + "' and type='"+ type + "'";
            DataTable dt = objcon.ReturnDataTableSql(sql);
            if (dt.Rows.Count > 0)
            {
                total = dt.Rows[0][0].ToString();
    ...
    public string ReturnCurPool(string username)
    {
        string Data = "0";
        try
        {
            clsConnection objcon1 = new clsConnection();
            string sql = "select max(cast(PoolNO as int)) as lastpool from TblPool2Matrix where reff='" + username + "'";
            DataTable dt = objcon1.ReturnDataTableSql(sql);
            if (dt.Rows.Count > 0)
            {
    ...
    public string ActivePool(string Packid)
    {
        string Data = "0";
        try
        {
            clsConnection objcon1 = new clsConnection();
            string sql = "select status from tblproduct where Packid='" + Packid + "'";
            DataTable dt = objcon1.ReturnDataTableSql(sql);
            if (dt.Rows.Count > 0)
            {
                Data = dt.Rows[0][0].ToString();
    ...
    public string RetrunUserName(string wallet)
    {
        string TotalTeamBusness = "0";
        try
        {
            clsConnection objcon1 = new clsConnection();
            string sql = "select username from register where name='" + wallet + "'";
            DataTable dt = objcon1.ReturnDataTableSql(sql);
            if (dt.Rows.Count > 0)
            {
                TotalTeamBusness = dt.Rows[0][0].ToString();
