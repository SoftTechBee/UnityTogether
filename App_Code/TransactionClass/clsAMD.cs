    ...
    public class clsAMD
    {
        ...
        public int WithdrawRequest(int RID, string Username, decimal Amount, string Remark,string IncomeType,string TranstionPass, string Transaction)
        {
            ...
            int status = ObjConnection.ExecuteProcedure("WithdrawRequest", ptmrid, ptmUsername, ptmAmount,
                ptmRemark, ptmIncomeType, ptmTranstionPass, ptmTransaction);
            return status;
        }
        #endregion
        #region Related WithdrawRequest
        ...
        #region Related to BoxPurchase
        public int BoxPurchase(string UserName, string HashCode, string BoxType, int BoxNo, decimal BoxAmount)
        {
            SqlParameter ptmUserName = new SqlParameter("@UserName", UserName);
            SqlParameter ptmHashCode = new SqlParameter("@HashCode", HashCode);
            SqlParameter ptmBoxType = new SqlParameter("@BoxType", BoxType);
            SqlParameter ptmBoxNo = new SqlParameter("@BoxNo", BoxNo);
            SqlParameter ptmBoxAmount = new SqlParameter("@BoxAmount", BoxAmount);

            return ObjConnection.ExecuteProcedure("[BoxPurchase]", ptmUserName, ptmHashCode, ptmBoxType, ptmBoxNo, ptmBoxAmount);



        }
        #endregion
  #region Related to BoxPurchaseNoofIds
  public int BoxPurchaseNoofIds(string UserName, string HashCode, string BoxType, int BoxNo, decimal BoxAmount,int TotalEntry)
  {
      SqlParameter ptmUserName = new SqlParameter("@UserName", UserName);
      SqlParameter ptmHashCode = new SqlParameter("@HashCode", HashCode);
      SqlParameter ptmBoxType = new SqlParameter("@BoxType", BoxType);
      SqlParameter ptmBoxNo = new SqlParameter("@BoxNo", BoxNo);
      SqlParameter ptmBoxAmount = new SqlParameter("@BoxAmount", BoxAmount);
      SqlParameter ptmTotalEntry = new SqlParameter("@TotalEntry", TotalEntry);

      return ObjConnection.ExecuteProcedure("[BoxPurchase_NoId]", ptmUserName, ptmHashCode, ptmBoxType, ptmBoxNo, ptmBoxAmount, ptmTotalEntry);
        ...
        #region Related to AutoWithdrawWithContract
        public int AutoWithdrawWithContractUpdate( string HashCode, string Transaction)
        {
            ...
            return ObjConnection.ExecuteProcedure("[AutoWithdrawByContract]", ptmHashCode, ptmTransaction);
