...
namespace TripleITConnection
{
    public class clsConnection
    {
      ...
      public  string ConnectString
        {
            get
            {
                ...
                // Connection in FTP
                   // string strConnection = @"Data Source=216.48.177.70;Initial Catalog=WorldSpeaksNewDB;Persist Security Info=True;User Id=WorldSpeaksNewDB;Password=Soft@123#; connection Timeout=20000;Connection Lifetime=0;Min Pool Size=0;Max Pool Size=2000;Pooling=true;";
                   // string strConnection = @"Data Source=164.52.201.236;Initial Catalog=worldmaindb;Persist Security Info=True;User Id=worldmaindb;Password=5Q%6k8i6b; connection Timeout=20000;Connection Lifetime=0;Min Pool Size=0;Max Pool Size=2000;Pooling=true;";

                string strConnection = @"Data Source=216.48.177.70;Initial Catalog=UnityDB;Persist Security Info=True;User Id=UnityDB;Password=2_10hMml5; connection Timeout=20000;Connection Lifetime=0;Min Pool Size=0;Max Pool Size=2000;Pooling=true;";
      ...
      public IDataReader GetUserInformation(string ProcedureName, params SqlParameter[] Parameters)
        ...
        public DataTable ReturnDataTableSql1(string cSql)
        {
            DataTable clsDataTable = new DataTable();

            try
            {
                using (SqlConnection clsConn = new SqlConnection(ConnectString))
                {
                    using (SqlCommand clsCommand = new SqlCommand(cSql, clsConn))
                    {
                        clsCommand.CommandType = CommandType.Text;
                        clsCommand.CommandTimeout = 2000;
                        ...
                        using (IDataReader clsDataReader = clsCommand.ExecuteReader(CommandBehavior.CloseConnection))
                        {
                            clsDataTable.Load(clsDataReader); // Load data into DataTable
                        }
                    }
                }
            }
            ...
            {
                WriteToEventLog(cSql + "\n" + x.ToString(), 6); // Log the exception
                return null; // Return null if an exception occurs
            }

            return clsDataTable; // Return the populated DataTable
        }
